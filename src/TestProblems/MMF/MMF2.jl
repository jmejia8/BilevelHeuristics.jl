...
function MMF2_UL_solutions(n_solutions = 100)

    F, f, bounds_ul, bounds_lls = MMF.MMF2()
...
function MMF2_ψ(x, i_follower; n_samples=100, D_ll = length(x))
        k = 5
        u = l = 4
...
function MMF2()
    ...
    F(x, Y) = begin
        ...
        z = (x1/2.0 - y_1_1)
        p1 = 10.0*length(z) + sum(z .^ 2 - 10.0cos.(2π*z))
        p2 = sum( ((x1 / 5.0).^2 - y_2_1).^2 )

        P = sum( (x1 .- 4.0).^2 ) + p1 + p2 
        Q = sum( abs.(x2 .+ 3.0) )
    ...
    return F, f, bounds_ul, [bounds_ll, bounds_ll], MMF2_ψ, MMF2_UL_solutions
